...
function calculatePi(rowID) {
    ...
    var A1value = getAllete("../../../xml/GlobalFiler/" + locus + ".xml", "a" + A1);
    var A2value = getAllete("../../../xml/GlobalFiler/" + locus + ".xml", "a" + A2);
    var B1value = getAllete("../../../xml/GlobalFiler/" + locus + ".xml", "a" + B1);
    var B2value = getAllete("../../../xml/GlobalFiler/" + locus + ".xml", "a" + B2);
    var pi = 0;

    if (condition_pp_pp(A1, A2, B1, B2)) {
        pi = (1 / 2) + 1 / (2 * Number(A1value));
    }

    if (condition_pp_pq(A1, A2, B1, B2)) {
        pi = (1 / 2) + 1 / (4 * Number(A1value));
    }
    ...
    if (condition_pq_pp_or_qq(A1, A2, B1, B2)) {
        if (A1 == B1) {
            ...
            pi = (1 / 2) + 1 / (4 * Number(A2value));
        }
    }

    if (condition_pq_pq(A1, A2, B1, B2)) {
        pi = (1 / 2) + (1 / (8 * Number(A1value))) + (1 / (8 * Number(A2value))) + (1 / (8 * Number(A1value)) * Number(A2value));
    }

    if (condition_pq_pr_or_qr(A1, A2, B1, B2)) {
        if (A1 == B1 || A2 == B2) {
            pi = (1 / 2) + 1 / (8 * Number(A1value));
        } else {
            pi = (1 / 2) + 1 / (8 * Number(A2value));
        }
    }
